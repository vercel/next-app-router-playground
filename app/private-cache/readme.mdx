...
- Products labeled **"Private Cache"** (blue border) link to pages that use `use cache: private` to enable runtime prefetching. The content is still dynamic, but it's prefetched
  when the static content of the page is also prefetched.
- Products labeled **"No Private Cache"** (gray border) link to pages that don't use `use cache: private`, meaning their recommendations will be loaded after navigation.
- **With Private Cache**: Personalized recommendations appear instantly (runtime prefetched).
- **Without Private Cache**: Recommendations show a loading skeleton, then appear after they're finsished loading. The loading starts when the link is clicked.

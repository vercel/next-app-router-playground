...
- Private cached content is automatically **runtime prefetched** — fetched ahead of navigation using the user's real session data.
...
async function Recommendations({ productId }: { productId: string }) {
  // Runtime prefetched automatically
...
- This demo uses `use cache: private` (currently experimental). Runtime prefetching of private cached content is automatic in dev mode.

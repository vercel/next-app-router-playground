...
- App Shell Upgrading is a built-in behavior of `cacheComponents` — there is no separate flag. Combine `'use cache'` with `generateStaticParams` and a `loading.tsx` to get a self-upgrading shell for unknown slugs.

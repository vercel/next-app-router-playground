...
- **Price data** is fetched with `use cache: remote` in `getProductPrice()` - accessed after `connection()`, so it's cached in a remote cache at runtime.
- Each price component calls `connection()` to ensure it's never included in the static shell.

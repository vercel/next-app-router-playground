...
- Runtime products (4–9) generate a unique slug per click, so you always see the streaming behavior. Visiting the same URL again loads instantly from cache.
...
- Product detail pages use `cacheLife({ revalidate: 3600 })` so the cache eventually expires.
